...
(function (global) {
    ...
    PostPromise.prototype.then = function(accept) {
        this.accept = accept;
        return this;
    };

    PostPromise.prototype.catch = function(reject) {
        this.reject = reject;
        return this;
    };

    PostPromise.prototype.succeed = function(access) {
        if (this.accept)
            this.accept(access);
    };

    PostPromise.prototype.fail = function(error) {
        if (this.reject)
            this.reject(error);
    };
    ...
    MIDIAccess = function(options) {
        ...
        cordova.exec(
            (data) => {
                switch (data.action) {
                    case "onReady":
                        _callback_onReady(data.sources, data.destinations);
                        break;

                    case "receiveMIDIMessage":
                        _callback_receiveMIDIMessage(data.index, data.time, data.data);
                        break;

                    case "addDestination":
                        _callback_addDestination(data.index, data.data);
                        break;

                    case "removeDestination":
                        _callback_removeDestination(data.index);
                        break;

                    case "addSource":
                        _callback_addSource(data.index, data.data);
                        break;

                    case "removeSource":
                        _callback_removeSource(data.index);
                        break;
    ...
    MIDIOutput.prototype.send = function( data, timestamp ) {
        ...
        cordova.exec((data) => {}, (error) => {}, 'WebMIDI', 'send', [outputData]);
    ...
    MIDIOutput.prototype.clear = function() {
        ...
        cordova.exec((data) => {}, (error) => {}, 'WebMIDI', 'clear', [outputClearData]);
    };
